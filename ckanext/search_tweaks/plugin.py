...
CONFIG_MOLECULE_QF = "ckanext.search_tweaks.molecule.qf"
...
MOLECULE_FILTER_RE = re.compile(
    r"(?:^|[\s(])\+?dataset_type\s*:\s*"
    r"(?:\"molecule\"|'molecule'|molecule)(?=$|[\s)])",
    re.IGNORECASE,
)
...
class SearchTweaksPlugin(plugins.SingletonPlugin):
    ...
    def before_search(self, search_params: SearchParams):
        ...
        molecule_search = _is_molecule_search(search_params)
        if molecule_search:
            _set_molecule_qf(search_params)
        else:
...
def _filter_queries(search_params: SearchParams) -> Iterable[str]:
    """Yield filter-query strings without assuming CKAN's ``fq`` shape."""
    fq = search_params.get("fq")
    if isinstance(fq, str):
        yield fq
    elif isinstance(fq, (list, tuple)):
        for value in fq:
            if isinstance(value, str):
                yield value


def _is_molecule_search(search_params: SearchParams) -> bool:
    """Return whether Solr is explicitly restricted to molecule packages.

    CKAN currently supplies ``+dataset_type:molecule`` for the molecule page.
    Matching the complete filter clause also accepts common serializations but
    avoids treating unrelated filters or unrestricted API searches as molecule
    searches.
    """
    return any(
        MOLECULE_FILTER_RE.search(fq)
        for fq in _filter_queries(search_params)
    )


def _set_molecule_qf(search_params: SearchParams) -> None:
    """Append configured molecule fields without replacing the existing qf.

    Molecule text is intentionally left unchanged and delegated to the Solr
    field analyzers.  In particular, this avoids adding fuzzy operators to
    InChIKeys, quoted phrases, and chemical or structure-search syntax.
    """
    query = search_params.get("q")
    if not isinstance(query, str) or not query.strip():
        return

    current_qf = search_params.get("qf")
    molecule_qf = tk.config.get(CONFIG_MOLECULE_QF, DEFAULT_MOLECULE_QF)
    if (
        isinstance(current_qf, str)
        and isinstance(molecule_qf, str)
        and molecule_qf.strip()
    ):
        search_params["qf"] += " " + molecule_qf.strip()
